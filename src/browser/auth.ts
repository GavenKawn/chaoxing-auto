import { launchBrowser, saveCurrentCookies, checkLoginStatus, closeBrowser } from './launcher.js';
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      if (!isTrustedChaoxingUrl(url)) {
        logger.error('检测到非官方学习通域名，已停止登录流程');
        return false;
      }
      ...
      if (userInfo && isTrustedChaoxingUrl(url)) {
        return true;
      }
